...
BASE_PATH = "/playpen-shared/mshuang/morris/morris/d9ef6cf1-f6c3-4956-a91e-adf409e105f0/dataset/wearable_activity_monitor/sleep/garmin_vivosmart5"
...
def build_empty_sample(pid):
    return {
        ...
        "sleep_stage": np.array([], dtype="str"),
        "patient_id": np.array([pid]),
        "is_missing": True,
    }


def load_sleep_json(path: str, pid: str) -> dict:
    ...
    for r in records:
        ...
        stage = r.get("sleep_stage_state", None)

        if t_start is None or stage is None:
            continue
    ...
    for col in ["sleep_stage", "patient_id"]:
...
def save_sleep_to_parquet(split_ids, save_path):
    result_list = []
    exist_count = 0
    missing_count = 0

    for split_id in tqdm(split_ids):
        path = f"{BASE_PATH}/{split_id}/{split_id}_sleep.json"
        sample = load_sleep_json(path, split_id)
    ...
    def fix_sample(sample):
        ...
        sample["sleep_stage"] = sample["sleep_stage"].tolist()
...
def save_sleep_all():
    ...
    save_sleep_to_parquet(train_ids, "../AI-READI-processed/sleep_train.parquet")

    print("val:", len(val_ids))
    save_sleep_to_parquet(val_ids, "../AI-READI-processed/sleep_valid.parquet")

    print("test:", len(test_ids))
    save_sleep_to_parquet(test_ids, "../AI-READI-processed/sleep_test.parquet")
