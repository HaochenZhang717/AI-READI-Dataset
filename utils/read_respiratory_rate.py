...
BASE_PATH = "/playpen-shared/mshuang/morris/morris/d9ef6cf1-f6c3-4956-a91e-adf409e105f0/dataset/wearable_activity_monitor/respiratory_rate/garmin_vivosmart5"
...
def build_empty_sample(pid):
    return {
        ...
        "respiratory_rate": np.array([], dtype=np.float32),
...
def load_respiratory_rate_json(path: str, pid: str) -> dict:
    ...
    for r in records:
        ...
        rr = r.get("respiratory_rate") or {}
        rr_val = rr.get("value", None)
        rr_unit = rr.get("unit", None)

        if t is None or rr_val is None:
            continue
    ...
    df = df.dropna(subset=["time_utc", "respiratory_rate"])
...
def save_respiratory_rate_to_parquet(split_ids, save_path):
    result_list = []
    exist_count = 0
    missing_count = 0

    for split_id in tqdm(split_ids):
        path = f"{BASE_PATH}/{split_id}/{split_id}_respiratoryrate.json"
        sample = load_respiratory_rate_json(path, split_id)
    ...
    def fix_sample(sample):
        ...
        sample["respiratory_rate"] = sample["respiratory_rate"].astype(float).tolist()
...
def save_respiratory_rate_all():
    ...
    save_respiratory_rate_to_parquet(train_ids, "../AI-READI-processed/respiratory_rate_train.parquet")

    print("val:", len(val_ids))
    save_respiratory_rate_to_parquet(val_ids, "../AI-READI-processed/respiratory_rate_valid.parquet")

    print("test:", len(test_ids))
    save_respiratory_rate_to_parquet(test_ids, "../AI-READI-processed/respiratory_rate_test.parquet")


if __name__ == "__main__":
    save_respiratory_rate_all()
